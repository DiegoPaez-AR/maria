...
const TOOLS = [
  ...
  { name: 'buscar_slots_comunes', description: 'Cruza calendars de varios usuarios y devuelve slots libres comunes. usuarios = lista de ids.',
    inputSchema: { type: 'object', additionalProperties: true, required: ['usuarios'], properties: { usuarios: { type: 'array', items: { type: 'integer' } }, ventana_inicio: s('hora ini, ej 9'), ventana_fin: s('hora fin, ej 19') } } },
  ...
  { name: 'configurar_caldav', description: 'Onboarding CalDAV (iCloud/Yahoo/Fastmail). server_url + username + password (app password).',
    inputSchema: { type: 'object', additionalProperties: true, required: ['server_url', 'username', 'password'], properties: { server_url: s(''), username: s(''), password: s('') } } },
  { name: 'iniciar_microsoft_auth', description: 'Paso 1 onboarding Microsoft/Outlook: genera la URL PKCE para que el user autorice.',
    inputSchema: { type: 'object', additionalProperties: true, properties: { usuarioId: { type: 'integer' } } } },
