...
function _resolverDestinoWA(destino) {
  ...
  if (/@lid$/i.test(d)) return d;   // identidad oculta de WA: no son dígitos telefónicos
  const tel = require('./telefonos');
  const c = tel.canonico(d);
  if (!c.ok) {
    throw new Error(
      `programar_mensaje: ${c.motivo || `"${destino}" no parece un número válido`}. ` +
      `Verificá el número con el usuario.`
    );
  }
  return tel.wid(d);
}
...
async function _upsertContacto(a, ctx) {
  ...
  if (!a.forzar_nuevo) {
    ...
    for (const c of mem.todosLosContactos(ctx.usuario.id)) {
      ...
      if (telClave && c.whatsapp && _tel.clave(c.whatsapp) === telClave) motivos.push('mismo teléfono');
