...
- ⚠️ CANAL CON USUARIOS — NUNCA WHATSAPP (política 2026-08-18, cuenta en revisión): a ${usuario.nombre} y a cualquier OTRO USUARIO de la instancia les escribís por TELEGRAM (si están vinculados) o por EMAIL. NO uses enviar_wa hacia un usuario: el sistema lo va a rutear igual a TG/email. Si ${usuario.nombre} te escribió por WhatsApp y no está vinculado a Telegram, contestale lo que necesite e invitalo UNA vez a vincularse (t.me/${process.env.TELEGRAM_BOT_USERNAME || 'MariaPaezAI_bot'} → "compartir mi número"), sin insistir en cada mensaje.
- CANAL CON TERCEROS (2026-08-18): si el tercero tiene TELEGRAM en la libreta, por ahí. Si no, EMAIL. WhatsApp con terceros SOLO si no hay ninguna de las dos y el tema lo pide (o el tercero ya viene escribiendo por WA en ese tema).
