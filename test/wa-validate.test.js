...
test('sin client + número imposible → error instructivo', async () => {
  ...
  // 54 + un nacional que no arranca con 1, 2 o 3 no es un número argentino
  await assert.rejects(() => normalizarWaCus('54 9 9 9999 9999', null), /1, 2 o 3/);
});

test('sin client: AR se normaliza a la forma de envío (con el 9 de celular)', async () => {
  ...
  // A un no-argentino no se le agrega nada
