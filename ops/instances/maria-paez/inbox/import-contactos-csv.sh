...
for (const c of lista) {
  ...
  if (!yaExiste) {
    try {
      ...
      creados++;
      detalleNuevos.push(c.nombre);
    } catch (err) {
      console.error(`  ERROR creando "${c.nombre}":`, err.message);
    }
    continue;
  }
  // Existe: actualizar solo lo que esté null
  const patch = {};
  ...
  try {
    // upsert con COALESCE (que preserva lo del DB si excluded.X es null) ya
    // hace lo correcto; pero para que NO pise un valor existente, mando solo
    // los campos del patch (y null para los que no quiero tocar).
    m.upsertContacto({
      usuarioId: OWNER.id,
      nombre: c.nombre,
      whatsapp: patch.whatsapp || null,
      email: patch.email || null,
      cumple: patch.cumple || null,
...
console.log(`Actualizados: ${actualizados} (solo campos que estaban null)`);
console.log(`Intactos:     ${intactos} (ya tenían toda la info)`);

if (detalleNuevos.length) {
  console.log('\nNuevos:');
  detalleNuevos.forEach(n => console.log('  +', n));
}
if (detalleAct.length) {
  console.log('\nActualizados:');
  detalleAct.forEach(n => console.log('  ~', n));
}

console.log('\n=== STATE FINAL ===');
console.log(`contactos privados Diego: ${m.contactosPrivados(OWNER.id).length}`);
console.log(`contactos públicos: ${m.contactosPublicos().length}`);
NODE_EOF
